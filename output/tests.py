...
class OutputTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        new_production1 = Production.objects.create(time=time_now + timedelta(minutes=15), produced=1, used=1, 
                                           production_meter_reading=1, grid_meter_reading=0, producer=prod1, grid_feed_in=0)
        max_consumption1 = Consumption.objects.create(time=time_now + timedelta(minutes=15), self_consumption=0.125, 
                                                     grid_consumption=1.5, consumption=1.625, meter_reading=1.625, rate=rate,
                                                     reduced_price=Decimal(4.375), grid_price=Decimal(60), price=Decimal(64.375),
                                                     saved=Decimal(0.625), production=new_production1, consumer=con1)
        gertrude_consumption1 = Consumption.objects.create(time=time_now + timedelta(minutes=15), self_consumption=0.125, 
                                                     grid_consumption=2, consumption=2.125, meter_reading=2.125, rate=rate,
                                                     reduced_price=Decimal(4.375), grid_price=Decimal(80), price=Decimal(84.375),
                                                     saved=Decimal(0.625), production=new_production1, consumer=con2)
        new_production2 = Production.objects.create(time=time_now + timedelta(minutes=32), produced=1.2, used=1, 
                                           production_meter_reading=2.2, grid_meter_reading=0.2, producer=prod1, grid_feed_in=0.2)
        max_consumption2 = Consumption.objects.create(time=time_now + timedelta(minutes=32), self_consumption=1, 
                                                     ...
                                                     reduced_price=Decimal(35), grid_price=Decimal(0), price=Decimal(35),
        ...
        gertrude_consumption2 = Consumption.objects.create(time=time_now + timedelta(minutes=32), self_consumption=0, 
                                                     grid_consumption=0, consumption=0, meter_reading=2.125, rate=rate,
        ...
        new_production3 = Production.objects.create(time=time_now + timedelta(minutes=15), produced=2, used=1.8, 
                                           production_meter_reading=2, grid_meter_reading=0.2, producer=prod2, grid_feed_in=0.2)
        angela_consumption = Consumption.objects.create(time=time_now + timedelta(minutes=15), self_consumption=0.4, 
                                                     grid_consumption=0, consumption=0.4, meter_reading=0.4, rate=rate,
        ...
        olaf_consumption = Consumption.objects.create(time=time_now + timedelta(minutes=15), self_consumption=0.6, 
                                                     ...
                                                     reduced_price=Decimal(21), grid_price=Decimal(0), price=Decimal(21),
        ...
        annalena_consumption = Consumption.objects.create(time=time_now + timedelta(minutes=15), self_consumption=0.8, 
                                                     grid_consumption=0, consumption=0.8, meter_reading=0.8, rate=rate,
                                                     reduced_price=Decimal(28), grid_price=Decimal(0), price=Decimal(28),
    ...
    def test_all_productions(self):
        ...
        self.assertEqual(response.content.producers_total_production, Decimal(4.2))
        self.assertEqual(response.content.producers_total_used, Decimal(3.8))
        self.assertEqual(response.content.producers_total_revenue, Decimal(246.75))
        self.assertEqual(response.content.producers_total_consumption, Decimal(6.55))
        self.assertEqual(response.content.producers_total_saved, Decimal(15.25))
    ...
    def test_production(self):
        ...
        self.assertEqual(response.content.total_production, Decimal(2.2))
        self.assertEqual(response.content.total_used, 2)
        self.assertEqual(response.content.self_usage_percentage, Decimal(0.9))
        self.assertEqual(response.content.consumers_total_revenue, Decimal(183.75))
        self.assertEqual(response.content.consumers_total_consumption, Decimal(4.75))
        self.assertEqual(response.content.consumers_total_saved, Decimal(6.25))
